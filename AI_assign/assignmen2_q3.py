...
extra_step_curve = np.zeros((len(learningRate), nrepetitions))
#averaged_extra_step = np.zeros((len(gamma)))

for x in range(len(learningRate)):
    for i in range(nrepetitions):
        learning_curve[i] = homing_nn(nTrials, nSteps, learningRate[x], epsilon, gamma)   
...
plt.plot(learningRate, means)
plt.fill_between(learningRate,means - errors, means + errors,color='gray', alpha=0.2)
